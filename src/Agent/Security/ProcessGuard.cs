...
public sealed class ProcessGuard(AgentOptions options)
{
    public void EnsureCanStop(Process process)
    {
        ...
        if (blocked.Contains(process.ProcessName, StringComparer.OrdinalIgnoreCase)) throw new UnauthorizedAccessException($"Tiến trình '{process.ProcessName}' là tiến trình hệ thống được bảo vệ, không thể tắt.");
    }
}

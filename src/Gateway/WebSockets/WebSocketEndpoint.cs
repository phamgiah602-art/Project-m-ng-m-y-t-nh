...
public sealed class WebSocketEndpoint(ConnectionManager connections, MessageRouter router, ILogger<WebSocketEndpoint> logger)
{
    public async Task HandleAsync(HttpContext context)
    {
        ...
        var subProtocol = context.WebSockets.WebSocketRequestedProtocols.Contains("bearer") ? "bearer" : null;
        ...
        try
        {
            ...
            while (socket.State == WebSocketState.Open)
            {
                ...
                await router.RouteAsync(Encoding.UTF8.GetString(stream.ToArray()), id, context.Connection.RemoteIpAddress?.ToString());
            }
        }
        catch (Exception ex) { logger.LogWarning(ex, "WebSocket {ConnectionId} bị ngắt", id); }
        finally { foreach (var session in connections.Remove(id)) await router.NotifyDisconnectAsync(session, id); if (socket.State == WebSocketState.Open) await socket.CloseAsync(WebSocketCloseStatus.NormalClosure, "Closed", CancellationToken.None); }
    }
}

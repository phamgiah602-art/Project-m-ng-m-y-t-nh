...
public sealed class AuthService(IUserRepository users, IAgentRepository agents, IOptions<JwtOptions> jwtOptions) : IAuthService
{
    ...
    public async Task<AuthResult> RegisterAsync(string username, string password)
    {
        ...
        if (username.Length is < 3 or > 50 || password is null || password.Length < 8) return new(false, null, "Tên đăng nhập tối thiểu 3 và mật khẩu tối thiểu 8 ký tự.");
    ...
    {
        username = username?.Trim() ?? string.Empty;
        if (string.IsNullOrEmpty(password)) return new(false, null, "Sai tên đăng nhập hoặc mật khẩu.");
        ...
        if (_hasher.VerifyHashedPassword(user, user.PasswordHash, password) != PasswordVerificationResult.Failed)
        {
            ...
            return new(true, CreateToken(user), "Đăng nhập thành công.");
        }

        if (user.LockedUntil > DateTime.UtcNow) return new(false, null, "Tài khoản đang bị khóa tạm thời do nhập sai quá 5 lần.");
        
        user.FailedLoginCount++;
        if (user.FailedLoginCount >= 5 && !user.IsAdmin)
        {
            user.LockedUntil = DateTime.UtcNow.AddMinutes(5);
            user.FailedLoginCount = 0;
        }
        await users.SaveAsync();
        return new(false, null, "Sai tên đăng nhập hoặc mật khẩu.");
    }
    ...
    private string CreateToken(AppUser user) { var claims = new List<Claim> { new(JwtRegisteredClaimNames.Sub, user.Id.ToString()), new(ClaimTypes.Name, user.Username) }; if (user.IsAdmin) claims.Add(new(ClaimTypes.Role, "admin")); return CreateToken(claims, TimeSpan.FromMinutes(_jwt.AccessTokenMinutes)); }

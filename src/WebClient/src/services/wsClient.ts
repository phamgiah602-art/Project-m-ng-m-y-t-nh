...
export class WsClient {
  ...
  connect(token: string) { this.token = token; this.close(); const url = new URL(gatewayBaseUrl.replace(/^http/, 'ws') + '/ws'); url.searchParams.set('access_token', token); this.state('connecting'); this.socket = new WebSocket(url); this.socket.onopen = () => { this.retry = 1000; this.state('connected'); }; this.socket.onmessage = event => { try { const message = JSON.parse(event.data) as Envelope; if (message.action === 'PING') this.send('PONG', {}, message.sessionId); this.listeners.forEach(listener => listener(message)); } catch { /* ignore malformed server packet */ } }; this.socket.onclose = () => { this.state('reconnecting'); this.timer = window.setTimeout(() => this.token && this.connect(this.token), this.retry); this.retry = Math.min(this.retry * 2, 30000); }; this.socket.onerror = () => this.socket?.close(); }
  ...
  send(action: string, payload: Record<string, unknown>, sessionId?: string, agentId?: string) { if (this.socket?.readyState !== WebSocket.OPEN) throw new Error('WebSocket chưa kết nối.'); this.socket.send(JSON.stringify({ type: 'COMMAND', action, sessionId, agentId, timestamp: new Date().toISOString(), payload })); }
